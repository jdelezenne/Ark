...
{
    /// A singly-linked list with stable node addresses and Ark naming conventions.
    /// @details `getLast` and `removeLast` are O(n). Prefer `LinkedList` when tail access is common.
...
#pragma region Constructors and Assignments
        ...
        SinglyLinkedList(Memory::Allocator& allocator = Memory::getDefaultAllocator())
            : allocator{allocator}
        {
        }

        SinglyLinkedList(InitializerList<T> initializerList, Memory::Allocator& allocator = Memory::getDefaultAllocator())
            : allocator{allocator}
        {
...
#pragma region Accessors
        ...
        /// Returns a reference to the last element. O(1) via cached tail.
        Reference getLast()
        {
            ARK_ASSERT(!isEmpty());
            return tail->value;
        ...
        /// Returns a const reference to the last element. O(1) via cached tail.
...
#pragma region Modifiers
        ...
        /// Removes and returns the last element, or none if empty. O(n).
        Option<T> removeLast()
        {
            if (isEmpty())
            {
        ...
        Iterator remove(Iterator position)
        {
            ...
            Node* prev = head;
            while (prev != nullptr && prev->next != position.node)
            {
                prev = prev->next;
            }
            ARK_ASSERT(prev != nullptr);
        ...
        void appendList(SinglyLinkedList& other)
        {
            if (other.isEmpty())
            {
                return;
        ...
        SinglyLinkedList splitAt(Iterator position)
        {
            SinglyLinkedList result(allocator);
            ...
            while (prev->next != position.node)
            {
                prev = prev->next;
                ++remaining;
            }

            result.head = position.node;
            result.tail = tail;
        ...
        SinglyLinkedList splitAt(SizeType index)
        {
            if (index >= count)
            {
                return SinglyLinkedList(allocator);
            }
        ...
        void swap(SinglyLinkedList& other)
        {
            ...
            {
                SinglyLinkedList thisCopy(*this);
                SinglyLinkedList otherCopy(other);
