...
namespace Ark::Collections
{
    namespace Internal
    {
        ...
        struct HashMapDefaultHash final
        {
            usize operator()(Key const& value) const
            {
                ...
                {
                    static_assert(Traits::isSame<Key, void>, "HashMap requires a hash function for this key type.");
                    return 0;
                }
            }
        };
    ...
    /// Hash map implementation with stable node addresses and Ark naming conventions.
    template <typename Key, typename T, typename Hash = Internal::HashMapDefaultHash<Key>, typename KeyEqual = EqualTo<Key>>
    ...
    private:
        ...
        struct Node
        {
            Pair<KeyStorageType const, T> value;
            Node* bucketNext{nullptr};
            Node* orderPrev{nullptr};
            Node* orderNext{nullptr};

            Node(Pair<KeyStorageType const, T> const& v)
                : value(v)
            {
            }

            Node(Pair<KeyStorageType const, T>&& v)
                : value(Ark::move(v))
            {
            }
            ...
            Node(K&& key, Args&&... args)
                : value(Ark::forward<K>(key), T(Ark::forward<Args>(args)...))
            {
            }
        };
...
#pragma region Constructors and Assignments
    ...
    public:
        ...
        HashMap(Memory::Allocator& allocator = Memory::getDefaultAllocator())
            : allocator{allocator}
            , buckets(DefaultBucketCount, nullptr, allocator)
        {
        }

        /// Constructs an empty map with a target bucket count.
        /// @param bucketCount Initial bucket count.
        explicit HashMap(SizeType bucketCount, Memory::Allocator& allocator = Memory::getDefaultAllocator())
            : allocator{allocator}
            , buckets(getNormalizedBucketCount(bucketCount), nullptr, allocator)
        {
        }

        HashMap(SizeType bucketCount, Hash const& hash, KeyEqual const& equal, Memory::Allocator& allocator = Memory::getDefaultAllocator())
            : allocator{allocator}
            , buckets(getNormalizedBucketCount(bucketCount), nullptr, allocator)
            , hashFunction(hash)
            , keyEqual(equal)
        ...
        HashMap(InputIterator first, InputIterator last, Memory::Allocator& allocator = Memory::getDefaultAllocator())
            : allocator{allocator}
            , buckets(DefaultBucketCount, nullptr, allocator)
        {
            insert(first, last);
        ...
        HashMap(InputIterator first, InputIterator last, SizeType bucketCount, Memory::Allocator& allocator = Memory::getDefaultAllocator())
            : allocator{allocator}
            , buckets(getNormalizedBucketCount(bucketCount), nullptr, allocator)
        {
            insert(first, last);
        ...
        HashMap(InitializerList<ValueType> init, Memory::Allocator& allocator = Memory::getDefaultAllocator())
            : allocator{allocator}
            , buckets(DefaultBucketCount, nullptr, allocator)
        {
            insert(init);
        ...
        HashMap(InitializerList<ValueType> init, SizeType bucketCount, Memory::Allocator& allocator = Memory::getDefaultAllocator())
            : allocator{allocator}
            , buckets(getNormalizedBucketCount(bucketCount), nullptr, allocator)
        {
            insert(init);
...
#pragma region Accessors and Modifiers
        ...
        void rehash(SizeType bucketCount)
        {
            ...
            while (node != nullptr)
            {
                ...
                const SizeType index = getBucketIndex(node->value.first, newBucketCount);
        ...
        /// Sets value.
        Pair<Iterator, bool> setValue(Key const& key, MappedType const& value)
        {
            return insertOrAssign(key, value);
        }

        /// Sets value.
        Pair<Iterator, bool> setValue(Key&& key, MappedType&& value)
        {
            return insertOrAssign(Ark::move(key), Ark::move(value));
        }

        /// Removes value.
        SizeType removeValue(Key const& key)
        {
            return remove(key);
        }
...
#pragma region Lookup
        ...
        /// Returns at.
        MappedType& getAt(Key const& key)
        {
        ...
        /// Returns at.
        MappedType const& getAt(Key const& key) const
        {
            Node* node = findNode(key);
            ARK_ASSERT(node != nullptr);
            return node->value.second;
        ...
        /// Returns or default.
        MappedType& getOrDefault(Key const& key, MappedType& defaultValue)
        {
            Node* node = findNode(key);
            return node != nullptr ? node->value.second : defaultValue;
        }

        /// Returns or default.
        MappedType const& getOrDefault(Key const& key, MappedType const& defaultValue) const
        {
            Node* node = findNode(key);
            return node != nullptr ? node->value.second : defaultValue;
        }
        ...
        /// Returns value or.
        MappedType getValueOr(Key const& key, MappedType defaultValue) const
        {
            Node* node = findNode(key);
            return node != nullptr ? node->value.second : Ark::move(defaultValue);
        }
...
#pragma region Insertion and Assignment
        ...
        Pair<Iterator, bool> tryEmplace(Key const& key, Args&&... args)
        {
            ...
            Node* node = createNode(key, Ark::forward<Args>(args)...);
            appendNode(node);
        ...
        Pair<Iterator, bool> tryEmplace(Key&& key, Args&&... args)
        {
            ...
            ensureInsertCapacity();
            Node* node = createNode(Ark::move(key), Ark::forward<Args>(args)...);
            appendNode(node);
            return {Iterator(node), true};
        }
        ...
        /// Inserts or assign.
        template <typename M>
        Pair<Iterator, bool> insertOrAssign(Key const& key, M&& obj)
        {
            Node* existing = findNode(key);
            if (existing != nullptr)
            {
                existing->value.second = Ark::forward<M>(obj);
                return {Iterator(existing), false};
            }

            ensureInsertCapacity();
            Node* node = createNode(key, Ark::forward<M>(obj));
            appendNode(node);
            return {Iterator(node), true};
        }
        ...
        /// Inserts or assign.
        template <typename M>
        Pair<Iterator, bool> insertOrAssign(Key&& key, M&& obj)
        {
            Node* existing = findNode(key);
            if (existing != nullptr)
            {
            ...
            ensureInsertCapacity();
            Node* node = createNode(Ark::move(key), Ark::forward<M>(obj));
            appendNode(node);
            return {Iterator(node), true};
        }
...
#pragma region Operators
        ...
        MappedType& operator[](Key const& key)
        {
        ...
        void swap(HashMap& other)
        {
            ...
            {
                HashMap thisCopy(*this);
                HashMap otherCopy(other);
...
#pragma region STL Compatibility API
    ...
    public:
        ...
        MappedType& at(Key const& key)
        {
            return getAt(key);
        }

        MappedType const& at(Key const& key) const
        {
            return getAt(key);
        }
        ...
        template <typename... Args>
        Pair<Iterator, bool> emplaceValue(Args&&... args)
        {
            return emplace(Ark::forward<Args>(args)...);
        }
...
#pragma endregion
    ...
    private:
        ...
        /// Creates a node.
        template <typename K, typename... Args>
        Node* createNode(K&& key, Args&&... args)
        {
            return allocator.make<Node>(Ark::forward<K>(key), Ark::forward<Args>(args)...);
        }
        ...
        {
            const SizeType bucketIndex = getBucketIndex(node->value.first, buckets.getCount());
    ...
    inline void swap(HashMap<Key, T, Hash, KeyEqual>& a, HashMap<Key, T, Hash, KeyEqual>& b)
    {
        a.swap(b);
    }
}
